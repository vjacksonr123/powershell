...
(SELECT [mdb].[dbo].[prp].[value] FROM [mdb].[dbo].[prp] WHERE ([mdb].[dbo].[prp].[label] = '(ITDeploy) Source Server Name' OR [mdb].[dbo].[prp].[label] = '(NTAdmin/ITDeploy) Source Server Name') AND [mdb].[dbo].[prp].[object_id] = [mdb].[dbo].[wf].[object_id] ) vmsource,
