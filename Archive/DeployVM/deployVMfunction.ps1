	...
	elseif ($OS -eq "SRV")
		{
			...
			elseif ($OSver -eq "XPUSER")
			{
				$vmAdminPassword = "App317config"
				$vmOSkey = "DPFQJ-RVH2C-QHF3V-229X3-38QK8"
				$OS = "DESK"
			}
			elseif ($OSver -eq "XPUSERT1")
			{
				$vmAdminPassword = "App317config"
				$vmOSkey = "DPFQJ-RVH2C-QHF3V-229X3-38QK8"
				$OS = "DESK"
			}
			elseif ($OSver -eq "XPUSERT3")
			{
				$vmAdminPassword = "App317config"
				$vmOSkey = "DPFQJ-RVH2C-QHF3V-229X3-38QK8"
				$OS = "DESK"
			}		
			elseif ($OSver -eq "WXP-SP2-X86")
			{
				$vmAdminPassword = "App317config"
				$vmOSkey = "DPFQJ-RVH2C-QHF3V-229X3-38QK8"
				$OS = "DESK"
...
function fnVMConfig([string]$vmname, [string]$OS, [string]$OSver, [string]$vmIPaddress, [string]$vmBOIPaddress, [string]$vmvcpus, [string]$vmram, [string]$vmdisk, [string]$vmhost, [string]$vmdatastore, [string]$vmfolder, [string]$vmcluster, [string]$vmresourcepool, $gui, $deploy, [string]$vmsource, $dr)
{
	...
	If($vmname -notlike 'LX*')
	{
		...
		Write-Host $vmname "deployed. Waiting 15 minutes to continue computer object setup."
		
		Start-Sleep -Seconds 900
				
		move-qadobject ("CONSECO\" + $vmname +"$") -NewParentContainer 'CONSECO.AD/Windows Servers'	  
	
		Write-Host "Waiting 5 minutes to continue VM post deploy setup."
		...
		Invoke-VMScript -VM $VM -HostUser root -HostPassword "delta347#" -GuestUser 'CONSECO\SyNT_Service_Account' -GuestPassword 'autoup' -ScriptType Powershell  -ScriptText "c:\windows\temp\VM.PostDeploySetupOnVM.ps1 $vmBOIPaddress $VMNIC1MAC $VMNIC2MAC"
		
		Write-Host "Waiting 5 minutes for final reboot."
		Start-Sleep -Seconds 300
		$VM | Restart-VMGuest
	...
	{
	Write-Host $vmname "deployed. Waiting 10 minutes to reboot VM and continue"
