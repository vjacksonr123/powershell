...
$dst = Connect-VIServer "ntvsvcp02.conseco.ad" -NotDefault
...
function Import-VMHosts {
	...
	foreach ($vmhost in (Import-Clixml "VMHosts.xml")) {
		if ($vmhost.Cluster) {
			Add-VMHost $vmhost.Name -Location (Get-Cluster $vmhost.Cluster -Location (Get-Datacenter $vmhost.Datacenter -Server $dst)) -Credential $HostCred
		} else {
			Add-VMHost $vmhost.Name -Location ($vmhost.Path | Get-HostLocation(Get-Datacenter $vmhost.Datacenter -Server $dst)) -Credential $HostCred
		}
	}
}
