...
$src = Connect-VIServer "ntvsvcp01.conseco.ad" -NotDefault
...
function Export-DrsRules {
	$DrsRules=@()
	ForEach ($c in (Get-Cluster -Server $src)) {
...
function Export-VMHosts {
	...
	$VMHosts = @()
	foreach ($h in Get-VMHost -Server $src) {
...
function Disconnect-Hosts {
	...
	Get-VMHost -Server $src | Set-VMHost -State "Disconnected" -Confirm:$false
}
