...
# Get the T-SQL 
$SqlQuery = "Select * FROM NT_DRSERVERINFO_RESTORE WHERE DRStatus = 'RESTORE'"
...
if ($nRecs -gt 0)
{
 Connect-VIServer -Server "vcenter.conseco.ad"
  ...
  ForEach ($Row In $DataSet.Tables[0].Rows)
  {
	...
	fnDeployVMfake $Row[0] $Row[1] $Row[2] $Row[3] $Row[4] $Row[5] $Row[6] $Row[7] $Row[8] $Row[9] $Row[10] $Row[11] $Row[12] $false $true "" $true
	Write-Host "*************** VM:" $Row[0] "*************** "
	Write-Host " "
  }
